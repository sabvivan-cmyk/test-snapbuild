...
const actions = [
  { label: 'Изменить заголовок', prompt: 'Сделай заголовок короче и конкретнее', status: 'Заголовок обновлён' },
  { label: 'Заменить изображение', prompt: 'Подбери другой визуальный акцент для первого экрана', status: 'Изображение заменено' },
  { label: 'Перестроить блок', prompt: 'Сделай композицию компактнее, сохранив иерархию', status: 'Композиция перестроена' },
  { label: 'Выбрать другой вариант', prompt: 'Покажи ещё один вариант в рамках дизайн-системы', status: 'Выбран вариант 02' },
]
...
export function AiControlSection() {
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLButtonElement>, index: number) => {
    ...
    else if (event.key === 'ArrowDown' || event.key === 'ArrowRight') next = (index + 1) % actions.length
  ...
  const previewTitle = activeAction === 0
      ...
      <div className="ai-control__surface">
        <div className="ai-control__commands">
          ...
          <div className="ai-control__command-footer">
            ...
            <button disabled={activeAction === null} onClick={() => setActiveAction(null)} type="button">Отменить</button>
          </div>
        </div>
        ...
        <div aria-hidden="true" className="control-editor">
          <div className="control-editor__bar">
            ...
            <div aria-hidden="true"><i className="is-active">Desktop</i><i>Tablet</i><i>Mobile</i></div>
          </div>
          ...
          <div className="control-editor__workspace">
            ...
            >
              <div className="control-preview__copy">
                ...
                <p>Готовая страница следует правилам бренда и сохраняет единую визуальную систему</p>
                <em>Подробнее</em>
              </div>
              <div className={`control-preview__visual${activeAction === 1 ? ' is-selected' : ''}`}>
                <i /><i /><i />
              </div>
              <div className={activeAction === 2 ? 'control-preview__layout-mark is-selected' : 'control-preview__layout-mark'}>Композиция</div>
            </div>
          </div>
        </div>
      </div>
