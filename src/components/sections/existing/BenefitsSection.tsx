    ...
    <section className="section benefits" id="benefits">
      ...
      <div className="comparison" role="region" aria-label="Сравнение подходов" tabIndex={0}>
        <div className="comparison__frame">
          <div aria-hidden="true" className="comparison__accent-border" />
          <table>
            <thead>
              <tr>
                <th>Особенности</th>
                <th className="comparison__accent">
                  <span className="comparison__brandname">снэпбилд</span>
                </th>
                <th>Claude + Figma MCP</th>
                <th>No-code платформы</th>
                <th>Cursor</th>
                <th>Традиционный</th>
              </tr>
            </thead>
              ...
              {rows.map(([name, ...values]) => (
                <tr key={name}>
                  ...
                  {values.map((value, index) => (
                    <td
                      className={index === 0 ? 'comparison__accent' : undefined}
                      key={`${name}-${index}`}
                    >
                      {value}
                    </td>
