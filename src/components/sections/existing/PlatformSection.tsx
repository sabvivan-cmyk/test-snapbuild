...
const principles = [
  {
    image: designSystem,
    desktopTitle: 'Дизайн-система — ядро платформы',
    mobileTitle: 'Дизайн-система Снэпбилд',
    text: 'Ваши компоненты, цвета и\u00a0шрифты — единственный источник стиля',
  },
  {
    image: configuration,
    desktopTitle: 'Гибкая конфигурация',
    mobileTitle: 'Гибкая конфигурация',
    text: 'Правила бренда задаются один раз — работают в\u00a0каждой генерации',
  },
  {
    image: compliance,
    desktopTitle: 'Соответствие по\u00a0умолчанию',
    mobileTitle: 'Соответствие по\u00a0умолчанию',
  ...
  return (
    <section className="section platform" id="product">
      <header className="section-header">
        <h2>
          <span className="wide-copy">Одна платформа — весь маркетинг</span>
      ...
      <div className="principles">
        {principles.map((item) => (
          <article className="principle" key={item.desktopTitle}>
            <img src={item.image} alt="" />
            <h3>
              <span className="wide-copy">{item.desktopTitle}</span>
              <span className="narrow-copy">{item.mobileTitle}</span>
            </h3>
            <p>{item.text}</p>
          </article>
        ))}
