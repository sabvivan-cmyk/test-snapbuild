...
export function RoadmapSection() {
  ...
  useEffect(() => {
    ...
    const release = () => { dragging = false; scroller.classList.remove('is-dragging') }
  ...
  return <section className="section roadmap" id="roadmap"><header className="section-header"><h2>Каждый день — новый релиз</h2><p>{'Приоритизируем бэклог для\u00a0ваших целей'}</p></header><div className="roadmap__scroller" ref={scrollerRef} tabIndex={0} role="region" aria-label="Дорожная карта"><ol className="roadmap__list">{releases.map(([date, title, text], index) => <li className={index < 8 ? 'is-reached' : ''} key={title}><h3>{title}</h3><p>{text}</p><time>{date}</time></li>)}</ol></div></section>
}
