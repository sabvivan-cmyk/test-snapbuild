...
export function HeroSection() {
  return <><section className="hero" id="top"><div className="hero__card"><div className="hero__inner"><div className="hero__copy"><h1>Платформа, где все создается в&nbsp;рамках вашего бренда и&nbsp;дизайн-системы</h1><p>Подключите дизайн-систему к Снэпбилду, чтобы каждый участник команды мог создавать профессиональные материалы в фирменном стиле за минуты, а не дни.</p><ButtonLink href="https://builder.snapbuild.ru/">Начать сейчас</ButtonLink></div><div className="hero__media"><img className="hero__visual" src={heroImage} alt="" /></div></div></div></section><section className="client-logos" aria-label="Компании, использующие Снэпбилд"><div className="client-logos__track">{[0, 1].map((copy) => <div className="client-logos__set" aria-hidden={copy === 1} key={copy}>{logos.map((item) => <img src={item.src} alt={copy === 0 ? item.alt : ''} key={item.alt} />)}</div>)}</div><p>С&nbsp;платформой работают команды, для&nbsp;которых бренд&nbsp;— закон</p></section></>
}
