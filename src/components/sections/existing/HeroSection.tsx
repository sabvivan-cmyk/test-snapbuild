...
const logos = [
  { src: logoOzon, alt: 'OZON' },
  { src: logoT2, alt: 't2' },
  { src: logoAvito, alt: 'Авито' },
  { src: logoCian, alt: 'Циан' },
  { src: logoLenta, alt: 'Лента' },
]
        ...
        <div className="hero__card">
          <div className="hero__inner">
            ...
            <div className="hero__media">
              <img className="hero__visual" src={heroImage} alt="" />
            </div>
          </div>
        </div>
      </section>
      <section
        className={`client-logos${logosRevealed ? ' is-revealed' : ''}`}
        aria-label="Компании, использующие Снэпбилд"
        ref={logosRef}
      >
        <div className="client-logos__track">
          {[0, 1].map((copy) => (
            <div className="client-logos__set" aria-hidden={copy === 1} key={copy}>
              {logos.map((item) => (
                <img src={item.src} alt={copy === 0 ? item.alt : ''} key={item.alt} />
              ))}
            </div>
          ))}
        </div>
